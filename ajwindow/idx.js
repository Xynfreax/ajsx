$$.require(['str'],function() {
  // Object.defineProperties($.ajwindow,);
...
function initResize(e) {
  ...
  if(count==1){
    thh.inidesf=thh.dom.offsetTop-thh.dom.getBoundingClientRect().top;//desfase
    inidesf=thh.inidesf;
    // inidesf=thh.dom.offsetTop-thh.dom.getBoundingClientRect().top;//desfase
  }else if(count>1){
    inidesf=thh.inidesf;
    console.log(thh.inidesf);
    // inidesf=0//parseInt((thh.dom.offsetTop-thh.dom.getBoundingClientRect().top)*(1));
  }
  console.log('inidesf ',inidesf);
...
elm.style.transform= 'translate('+newWidth+', '+(newHeight-inidesf)+'px)';
...
function stopResize(e) {
  ...
  window.removeEventListener('mouseup', stopResize, false);
    ...
    ,setAutofocus:function() {
      let th=this;
      let dautofocus=th.dom.querySelector("*[autofocus]");
      // console.log(dautofocus);
      // alert(JSON.stringify(dautofocus));
      // if(!dautofocus){
    ...
    ,show:async function() {
      ...
      // }
      if(!this.inlogin){
        // alert('PORQUE LLEGO HASTA AQUI');
        if(window.USUARIO_ACTUAL==undefined){
          // alert('ESTA INDEFINIDO')
          let resp=await $_.query('listaPermisosxusuario');
          // alert(JSON.stringify(resp.data));
          // alert(JSON.stringify(resp.data));
          // alert('PASE EL LOGIN')
          if(resp.data!==undefined){ //window.USUARIO_ACTUAL==undefined &&
            window.USUARIO_ACTUAL=resp.data;
          }
    ...
    ,customProps:{
      ...
      ,showCloseIcon:{set:function(v){
        if(v==false){
          ...
          this.mytitlebar.myclose.display='inline-flex'
        }
      },configurable:true}
